...
/*   Updated: 2023/09/13 18:45:06 by dapaulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Cat::Cat(void) : Animal(), brain(new Brain) {
	this->_type = "Cat";
	std::cout << GRNHB "[ Cat - Create a " BBLK << Animal::_type << BLK GRNHB " ]" reset << std::endl;
}

Cat::~Cat(void) {
	std::cout << REDHB "[ Cat - Destroy a " BBLK << Animal::_type << BLK REDHB " ]" reset << std::endl;
	delete this->brain;
}
...
Cat::Cat(Cat const &cat) {
	...
	std::cout << YELHB"[ Cat - Constructor copy " BBLK << Animal::_type << BLK YELHB " ]" reset << std::endl;
	std::cout << CYNHB "[ Copy < " BBLK << &*(cat.getBrain()) << BLK CYNHB " > to < " BBLK  << &this->brain << BLK CYNHB " > ]" reset << std::endl;
}

Cat::Cat(Animal const *cat) {
...
Cat	&Cat::operator=(Cat const &rhs) {
	...
	std::cout << MAGHB "[ Cat - Constructor assigment " BBLK << Animal::_type << BLK MAGHB " ]" reset << std::endl;
	return *this;
}
